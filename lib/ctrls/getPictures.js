...
module.exports = function (location, city) {
  ...
  Q.spawn(function *() {
    ...
    try {
      let res = yield rp({
        url: CITY_URL.replace(/:location/, location).replace(/:city/, encodeURIComponent(city)),
        json: true
      })
      ...
      if (
        ...
        res.results[0].photos &&
        res.results[0].photos.length > 0 &&
        res.results[0].photos[0].photo_reference
      ) {
